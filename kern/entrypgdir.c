...
pde_t entry_pgdir[NPDENTRIES] = {
	// Map VA's [0, 4MB) to PA's [0, 4MB)	//代表pgdir数组中第0个元素的值 = value
	[0]
		= ((uintptr_t)entry_pgtable - KERNBASE) + PTE_P,	//entry_pgtable在符号表中是0xf00.... 此处在编译完成后的代码段数据是0x000....
	// Map VA's [KERNBASE, KERNBASE+4MB) to PA's [0, 4MB)
	[KERNBASE>>PDXSHIFT]		//代表pgdir数组中第KERNBASE>>PDXSHIFT个元素的值 = value
...
//bit31-12代表页帧地址即用于定位物理地址空间中的一个页面  bit11-0为控制位
__attribute__((__aligned__(PGSIZE)))
pte_t entry_pgtable[NPTENTRIES] = {
